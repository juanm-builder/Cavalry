...
Renderer-owned behavior lives in `apps/mac/src/renderer/features/drafts/`: the React review route, explicit action callbacks, source navigation, confirmations, and controller effects.
...
Do not import Electron, Node-only modules, preload bridges, filesystem access, provider clients, `window`, or `document` here. Do not change checkpoint apply semantics, action-plan validation, rollback/diff shapes, API auth/scopes, or OpenAPI-visible behavior without checkpoint/API parity coverage.
