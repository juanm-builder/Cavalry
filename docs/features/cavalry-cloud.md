...
1. Choose the permanent iOS bundle identifier and register its Apple App ID with **Sign in with Apple** enabled as the primary identifier.
2. Create an Apple Services ID for browser OAuth and associate it with that primary App ID. The Services ID can use a separate reverse-domain value such as `com.example.cavalry.auth`.
