-- Treat optimistic-concurrency conflicts as an HTTP precondition failure.
--
-- SQLSTATE 40001 means serialization_failure. PostgREST may retry transactions
-- carrying that code, but a stale workbook revision is a deterministic product
-- conflict that cannot succeed without new client input. PT412 preserves the
-- existing conflict message while mapping the response to HTTP 412.
