...
const CLARIFICATION_TOOL = Object.freeze({
  ...
  description:
    'Pause the task and ask the user one focused follow-up question when required information is missing. Offer concise choices when they would help the user answer quickly.',
  parameters: {
    type: 'object',
    properties: {
      question: {
...
export function buildCavalryAssistantInstructions({ activeRouteId, today } = {}) {
  ...
  return [
    ...
    'When required information is missing or a meaningful choice belongs to the user, call request_clarification before any action tool instead of assuming. Ask one focused question and offer concise options when useful. Never combine request_clarification with another tool call.',
