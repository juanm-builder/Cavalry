// Owns Supabase PKCE authentication and keeps every credential in the privileged desktop host.
'use strict';
...
async function hasEncryptedCloudState(readFile, filePath) {
  try {
    ...
    return !!(
      document &&
      document.version === 1 &&
      ['cavalry-os-safe-storage-v1', 'electron-safe-storage'].includes(document.encryption) &&
      values &&
